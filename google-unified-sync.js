(function installSorayaGoogleUnifiedSync(){
  ...
  function openItem(x){
    if(!x)return;const title=$('#detailTitle'),ey=$('#detailEyebrow'),content=$('#detailContent'),modal=$('#detailModal'),back=$('#detailBackdrop');if(!title||!content||!modal||!back)return;
  ...
  function renderCalendarCells(){
    ...
    days.forEach((cell,i)=>{const d=new Date(p.y,p.m,1-start+i),dayIso=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;const visible=items.filter(x=>x.item_date===dayIso&&!x.completed&&!(x.soraya_id&&['tasks','events'].includes(x.soraya_table)));const rows=visible.slice(0,3);for(const x of rows){const n=document.createElement('div');n.className=`soraya-google-item ${x.source==='tasks'?'task':''}`;n.textContent=`G · ${x.title}`;n.title=`${sourceLabel(x)}${x.start_time?' · '+time5(x.start_time):''}`;n.addEventListener('click',e=>{e.stopPropagation();openItem(x)});cell.appendChild(n)}if(visible.length>3){const more=document.createElement('div');more.className='soraya-google-more';more.textContent=`+${visible.length-3} de Google`;cell.appendChild(more)}})
  }
  function renderToday(){
    const view=$('#view-dashboard');if(!view)return;const today=iso(new Date()),rows=items.filter(x=>x.item_date===today&&!x.completed&&!x.soraya_id).slice(0,6);let box=$('#sorayaGoogleToday');if(!rows.length){box?.remove();return}if(!box){box=document.createElement('section');box.id='sorayaGoogleToday';box.className='soraya-gsync-today';const hero=view.querySelector('.jf-exec-hero,.hero');if(hero)hero.insertAdjacentElement('afterend',box);else view.prepend(box)}box.innerHTML=`<div class="soraya-gsync-today-head"><strong>G · También en tu Google hoy</strong><span class="soraya-gsync-badge">${rows.length}</span></div><div class="soraya-gsync-mini">${rows.map(x=>`<button type="button" data-gsync-today="${esc(x.id)}"><b>${esc(x.title)}</b><span>${x.start_time?esc(time5(x.start_time))+' · ':''}${esc(sourceLabel(x))}</span></button>`).join('')}</div>`;box.querySelectorAll('[data-gsync-today]').forEach(n=>{n.onclick=()=>openItem(items.find(x=>x.id===n.dataset.gsyncToday))});
  }
  function renderSettings(){
    const anchor=$('#googleConnectBtn');if(!anchor)return;let box=$('#sorayaGoogleSyncSettings');if(!box){box=document.createElement('div');box.id='sorayaGoogleSyncSettings';box.className='soraya-gsync-status';anchor.parentElement?.insertAdjacentElement('afterend',box)}const stamp=Number(localStorage.getItem(LS_LAST)||0);box.innerHTML=`<strong>Sincronización Soraya</strong><br>${stamp?'Última actualización: '+new Intl.DateTimeFormat('es-UY',{dateStyle:'short',timeStyle:'short'}).format(new Date(stamp)):'Todavía no sincronizado.'}<br><span class="soraya-gsync-link">Calendar + Google Tasks</span>${lastResult?.needs_reconnect?'<br>⚠️ Reconectá Google para autorizar Tasks.':''}`;
  }
