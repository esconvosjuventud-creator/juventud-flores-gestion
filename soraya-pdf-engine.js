(function(root,factory){
  ...
  class PDFDoc{
    ...
    text(value,x,y,opts){const lines=Array.isArray(value)?value:[value],lh=(this.fontSize*1.18)/PT_PER_MM;lines.forEach((line,i)=>{let xx=Number(x),yy=Number(y)+i*lh;const o=opts||{};if(o.align==='right')xx-=this.getTextWidth(line);else if(o.align==='center')xx-=this.getTextWidth(line)/2;const c=this.textColor.map(n=>n.toFixed(3)).join(' '),font=this.font==='bold'?'/F2':'/F1';this.pages[this.pageIndex].push(`BT ${font} ${this.fontSize.toFixed(2)} Tf ${c} rg 1 0 0 1 ${(xx*PT_PER_MM).toFixed(3)} ${(PAGE_H-yy*PT_PER_MM).toFixed(3)} Tm <${hexText(line)}> Tj ET`)});return this}
    toUint8Array(){if(!this.image)throw new Error('Falta la hoja membretada institucional');const pageCount=this.pages.length,objs=[];objs[1]=ascii('<< /Type /Catalog /Pages 2 0 R >>');const kids=[];for(let i=0;i<pageCount;i++)kids.push(`${6+i*2} 0 R`);objs[2]=ascii(`<< /Type /Pages /Kids [${kids.join(' ')}] /Count ${pageCount} >>`);objs[3]=ascii('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica /Encoding /WinAnsiEncoding >>');objs[4]=ascii('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold /Encoding /WinAnsiEncoding >>');const imgHead=ascii(`<< /Type /XObject /Subtype /Image /Width ${this.imageWidth} /Height ${this.imageHeight} /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length ${this.image.length} >>\nstream\n`),imgTail=ascii('\nendstream');objs[5]=concat([imgHead,this.image,imgTail]);for(let i=0;i<pageCount;i++){const pObj=6+i*2,cObj=7+i*2,content=ascii(this.pages[i].join('\n')+'\n');objs[pObj]=ascii(`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${PAGE_W.toFixed(3)} ${PAGE_H.toFixed(3)}] /Resources << /Font << /F1 3 0 R /F2 4 0 R >> /XObject << /Im1 5 0 R >> >> /Contents ${cObj} 0 R >>`);objs[cObj]=concat([ascii(`<< /Length ${content.length} >>\nstream\n`),content,ascii('endstream')])}const max=5+pageCount*2,parts=[ascii('%PDF-1.4\n%\xE2\xE3\xCF\xD3\n')],offsets=new Array(max+1).fill(0);let pos=parts[0].length;for(let i=1;i<=max;i++){offsets[i]=pos;const h=ascii(`${i} 0 obj\n`),t=ascii('\nendobj\n');parts.push(h,objs[i],t);pos+=h.length+objs[i].length+t.length}const xrefPos=pos;let x=`xref\n0 ${max+1}\n0000000000 65535 f \n`;for(let i=1;i<=max;i++)x+=String(offsets[i]).padStart(10,'0')+' 00000 n \n';x+=`trailer\n<< /Size ${max+1} /Root 1 0 R >>\nstartxref\n${xrefPos}\n%%EOF\n`;parts.push(ascii(x));return concat(parts)}
